      ...
      success:function(resposta){
				...
				resposta.forEach(function(auser){
					...
					var loaduser = {
						...
						lat: auser.address.geo.lat,
						lng: auser.address.geo.lng,
						ride: ride5.toString(),
		        days: days6.toString(),
		        posts: cp[t],
		        albums: ca[t],
								...
								{filteredUsers.map((users, indexu) => {
									...
									return (
										<tr key={users.id} className={users.newClass}>
											...
											<td>{users.name}</td>
											<td className="info-email"><a href={`${users.email}`} target="_blank" rel="noopener noreferrer">{users.email}</a></td>
											<td className="info-city"><a href={`https://www.google.com/maps/${users.lat},${users.lng},15z`} target="_blank" rel="noopener noreferrer">{users.city}</a></td>
											<td>{users.ride}</td>
											<td>{users.days}</td>
